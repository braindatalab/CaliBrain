...
- **Aggregation** – Collects the saved `posterior_summary*.h5` files, groups them exactly by experiment metadata, and writes consolidated train/test datasets so you can split whole runs (never partial sources) for downstream analysis.
