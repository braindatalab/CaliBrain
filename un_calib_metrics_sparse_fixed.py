...
# =============================================================================
# UNCERTAINTY QUANTIFICATION AND CALIBRATION METRICS FOR SPARSE SOLVERS 
# =============================================================================
import numpy as np
...
def compute_calibration_curve_sparse(x_hat, posterior_cov, active_set, x_true, confidence_levels=None):
    """
    Compute the calibration curve for sparse estimators (e.g., gamma-MAP) using only active sources.

    Parameters:
    - x_hat: (n_sources, n_times) predicted source time courses
    - posterior_cov: (n_active, n_active) covariance matrix for active sources
    - active_set: list or array of indices of active sources
    ...
    - empirical_coverages: np.ndarray of empirical coverages (only over active set)
    - std: np.ndarray of posterior std devs (only over active set)
    """
    if confidence_levels is None:
        confidence_levels = np.linspace(0.5, 0.95, 10)

    
    # Average the amplitudes first, then select active sources:
    x_hat_avg = np.mean(x_hat, axis=1)[active_set]
    x_true_avg = np.mean(x_true, axis=1)[active_set]
    # Apply reduced variance when averaging over time:
    posterior_variance = np.diag(posterior_cov)
    #std = np.sqrt(posterior_variance)
    n_times = x_hat.shape[1]
    std = np.sqrt(posterior_variance/ n_times)  # variance scaling for time-averaging
...
def compute_uncertainty_and_calibration_metrics(nominal_confidences, empirical_coverages, std, active_set):
    """
    Compute scalar metrics for uncertainty and calibration.

    Parameters:
    - nominal_confidences (np.ndarray): Nominal confidence levels.
    - empirical_coverages (np.ndarray): Corresponding empirical coverages.
    - std (np.ndarray): Posterior standard deviations for all sources.
    - active_set (array-like): Indices of active sources.
    ...
    # std is already computed only for active_set
    ...
    return {
        ...
        "Mean Posterior Std (Active)": mean_std
    }



nom_conf, emp_cov, std = compute_calibration_curve_sparse(x_hat, posterior_cov, active_set, x_true = x)
plot_calibration_curve(nom_conf, emp_cov)
metrics = compute_uncertainty_and_calibration_metrics(nom_conf, emp_cov, std, active_set)
print("Uncertainty and Calibration Metrics:", metrics)
